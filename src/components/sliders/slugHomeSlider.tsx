...
import SlugCardOnSlider  from "../cards/slugCardOnSlider";
...
const SlugHomeSlider = ({ settings, data, headerData }: ISliderProps): JSX.Element => {
        ...
        <div className="absolute top-[352px] lg:top-[30rem] z-10  w-full px-12 items-center hidden sm:block">
          <div className="flex justify-between">
            <div className="flex gap-3">
                ...
                <button onClick={()=>playButton(data?.[currentSlide])} className="text-white bg-[#2A96FA] w-36 h-16 rounded flex justify-center items-center gap-2 text-[28.7px]">
                  <FaPlay className="w-[20.09px] h-[22.96px]" />Play</button>

              <button className="text-white bg-[#5858584D] bg-opacity-10 w-32 h-16 rounded flex justify-center items-center gap-2 text-[20px]">
                More Info
              </button>
              <button onClick={() => handleFavorite(data?.[currentSlide]._id)} className="text-[#AAAAAA] bg-opacity-10 text-[23.91px] h-16 rounded flex justify-center items-center gap-2">
                <IoAddCircleSharp size={40} className="text-[#AAAAAA]" />
                Add to My List
              </button>
            </div>
            ...
            <div className="flex gap-3 mt-10">
              <button
                ...
                className="text-white bg-white bg-opacity-10 w-16 h-16 rounded flex justify-center items-center gap-2"
              >
                <HiChevronLeft size={60} className="text-white" />
              </button>
              <button
                onClick={() =>
                ...
                className="text-white bg-white bg-opacity-10 w-16 h-16 rounded flex justify-center items-center gap-2"
              >
                <HiChevronRight size={60} className="text-white" />
              </button>
              <button
                onClick={() => setIsMute(!isMute)}
                className="text-white bg-opacity-10 w-16 h-16 rounded flex justify-center items-center gap-2"
              >
                {isMute ? (
                  <MdVolumeOff size={30} className="text-[#505A78]" />
                ) : (
                  <MdVolumeUp size={30} className="text-[#505A78]" />
                )}
              </button>
            </div>
          </div>
        </div>
        <div className="absolute left-1/2 z-10 transform -translate-x-1/2 -translate-y-1/2 top-[34rem]">
